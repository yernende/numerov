...
double inline static V(
  double radius, double energy, int l, int atomic_number,
  InterpolationData *potential_interpolation_data, double potential_shift
) {
  double result;
...
int compute_radial_part_numerov_dense(
  ...
  InterpolationData *potential_interpolation_data, double potential_shift, double h=1e-5
) {
  ...
  for (int i = 1; i < points_amount; i++) {
    ...
    f[i] = (1 - (h * h) / 12.0 * V(x_dense[i], energy, l, atomic_number, potential_interpolation_data, potential_shift));
  }
...
void static compute_radial_part_numerov_normalized(
  ...
  int points_amount, InterpolationData *potential_interpolation_data, double potential_shift
) {
  double *x_dense;
  double *y_dense;

  int dense_points_amount = compute_radial_part_numerov_dense(
    energy, l, atomic_number, radius_max, x_dense, y_dense, potential_interpolation_data, potential_shift
  );
...
extern "C" void compute_radial_part(
  ...
  double *potential_x, double *potential_y, int potential_points_amount, double potential_shift,
  ...
  compute_radial_part_numerov_normalized(
    ...
    &potential_interpolation_data, potential_shift
  );
